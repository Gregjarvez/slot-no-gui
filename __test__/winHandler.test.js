  ...
  test('reduce a stateObject', () => {
    return winHandlerDb.find('winHandler', (winHandler) => {
      return actualsDb.find('actual', ({grid, stateObject}) => {
        ...
        const stateObjectAfterWin = matchFound(stateObject)
        const stateObjectAfterLose = noMatchFound(stateObject)

        expect(stateObjectAfterLose).not.toEqual(stateObject)
        expect(stateObjectAfterWin).not.toEqual(stateObject)
      })
    })
  })
})